$(document).ready(() => {
  ...
  var selectizePacientes = new SelectSelectize("#select-sisnot-pacientes", {
    ...
    render: {
      item: function (item, escape) {
        const date = new Date(item.data_nascimento);
        const formatedDate =
          date.getDate() +
          "/" +
        ...
        <div class='d-inline-flex flex-column pb-2'>
          ${
            item.nome ? `<span class="nome"> ${escape(item.nome)}</span>` : ""
          } ${
          item.registro
            ? `<span class="text-secondary text-sm registro" style="line-height:10px"> Registro: ${escape(
                item.registro
              )} Data de nascimento: ${escape(formatedDate)} </span>`
            : ""
        }
        </div>
        `;
  ...
  $(".sisnot-expandable-row button").on("click", (e) => {
    const currentRow = $(e.target).closest(".sisnot-expandable-row");
    const expandableBody = $(currentRow).next(".expandable-body");
    const isDisplay = !$(expandableBody).hasClass("d-none");
    const content = $(expandableBody).find(".expandable-content");
  ...
  function handleSearch(e) {
    ...
    $("#sisnot-section-filter .nav-link.active").removeClass("active");
    $('#sisnot-section-filter .nav-link[data-filter=""]').addClass("active");

    filterNotification(value);
  }

  $("#sisnot-buscar-notificacao").on("input", handleSearch);
  $("#sisnot-btn-buscar").on("click", handleSearch);

  function filterNotification(value) {
    const cardBody = $("#sisnot-notificacao-container");

    if (value === "") {
      $(cardBody).find("tr").show();
      return;
    }
    ...
    $(cardBody)
      .find("tbody tr")
      .each((index, element) => {
        const status = $(element).data("status")?.toLowerCase();
        const text = $(element).text().toLowerCase();

        if (status?.includes(value) || text.indexOf(value) > -1) {
          $(element).show();
          return;
        }
  ...
  $("#sisnot-section-filter").on("click", ".nav-link", (e) => {
    $("#sisnot-section-filter .nav-link.active").removeClass("active");
    const currentFilter = $(e.target);
    currentFilter.addClass("active");
    const value = currentFilter.data("filter")?.toLowerCase();
    $("#sisnot-buscar-notificacao").val("");
    filterNotification(value);
  });
  ...
  $("#sisnot-buscar-incidente").on("click", () => {
    let startDate = calendarNotificacoesSisNot.selectedDates[0]
      ? removeTime(formatDate(calendarNotificacoesSisNot.selectedDates[0]).date)
      : null;
    let endDate = calendarNotificacoesSisNot.selectedDates[1]
      ? removeTime(formatDate(calendarNotificacoesSisNot.selectedDates[1]).date)
      : null;
  ...
  $(".form-incidente :radio, .form-incidente :checkbox").on("change", (e) => {
    ...
    const uncheckedInputs = $(textContainer).find(
      ".adicional-text-radio:not(:checked)"
    );
    ...
    const inputText =
      $(e.target).is(":checked") &&
      $(e.target).closest(".input-group").find(".adicional-text");
    if (inputText) {
  ...
  function validDetailsForm(e) {
    ...
    const validInput = (input) => {
      ...
      if (
        input.is(":checkbox:not(:checked)") ||
        input.is(":radio:not(:checked)")
      )
        return false;

      if (input.is(".adicional-text-radio")) return false;
    ...
    $(".form-incidente :input:not(:submit):not(.btn)").each(
      (index, element) => {
        const input = $(element);
  ...
  function validIncidenteForm(e) {
    ...
    if (!adicionalsTextIsValid) return false;
    ...
    switch (inputType) {
      case "radio":
        if (subquestionId !== "" && currentInput.is(":checked")) {
          const subquestionContainer = form.find(
            `[data-resposta='${subquestionId}'].subpergunta-container`
          );
          const questionId = currentInput.data("pergunta");

          const sameQuestions = form.find(
            `input:radio[data-pergunta='${questionId}']`
          );
  ...
  function validSelectize(selectize) {
    ...
    if (selectize.getValue() === "") {
      if (formGroup.find(".error-selectize").length > 0) return false;
      formGroup.append(
        '<label class="error-selectize" style="color:#ff6565; font-weight:normal !important;">Este campo é obrigatório</label>'
      );
      return false;
    } else {
  ...
  $("#sisnot-incidente-72h").on("change", (e) => {
    ...
    if (checkboxState) {
      ...
      selectizePacientes.settings.placeholder =
        "Selecione o paciente do incidente";
    }
  ...
  function getDashboardSisNot(incidentes = []) {
    ...
    let dataInicio = dashboardCalendarNS.selectedDates[0]
      ? formatDate(dashboardCalendarNS.selectedDates[0])
      : null;
    let dataFim = dashboardCalendarNS.selectedDates[1]
      ? formatDate(dashboardCalendarNS.selectedDates[1])
      : null;
  ...
  function renderChartsSisNot(dataPoints) {
    ...
    let chartMonth = new ChartMonth(
      "notificacoes-mes",
      dataPoints["notificacoes-mes"].data,
      dataPoints["notificacoes-mes"].media,
      CHART_MONTH
    );
    ...
    let chartWeek = new ChartWeek(
      "notificacoes-semana",
      dataPoints["notificacoes-semana"],
      CHART_WEEK
    );
    ...
    let chartSector = new ChartColumn(
      "notificacoes-setor",
      dataPoints["notificacoes-setor"],
      CHART_SECTOR
    );
    ...
    let chartRanking = new ChartPie(
      "ranking-incidentes",
      dataPoints["ranking-incidentes"],
      CHART_RANKING,
      "light2"
    );
