...
$(document).ready(() => {
    ...
    $('.modal-setor-paciente').on('click', async (e) => {
        const button = $(e.target);

        const setor = button.data('setor');
    ...
    $('#azophi-search-convenios').on('click', function () {
        ...
        $.ajax({
            ...
            .done(response => renderCharts(response))
    ...
    if ($('#azophi-internacao-convenios').length > 0) getCharts();
    ...
    const chart = {
        ...
        prontoAtendimentoConvenios: (data) => {
            ...
            Object.entries(data).filter(data => data[1].length > 0).map((content) => {
